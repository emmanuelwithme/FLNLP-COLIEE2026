...
class ModernBERTContrastive(nn.Module):
    def __init__(self, model_name: str, device, temperature: float = 0.55555):
        super().__init__()
        # 與 inference.py 保持一致的 encoder_kwargs 設定；bf16 由 Trainer 的 AMP 控制，權重維持 fp32
...
def main():
    ...
    ckpt_dir = get_env_path("TASK1_BF16_BASE_ENCODER_DIR", required=True)
    ...
    finetune_data_dir = get_env_path("TASK1_BF16_FINETUNE_DATA_DIR", required=True)
    base_output_dir = get_env_path("TASK1_BF16_MODEL_ROOT_DIR", required=True)
    ...
    args = TrainingArguments(
        ...
        bf16=True,
        tf32=False,
        learning_rate=7.5e-6,           # 給 encoder / projector
        num_train_epochs=20,
        warmup_ratio=0.1,
        lr_scheduler_type="cosine",
        optim="adamw_torch_fused",  # 使用穩定的 AdamW 以配合 bf16 訓練
